...
export class Oodle {
    ...
    static decompress(src: Buffer, dstLen?: Buffer | number, dst?: Buffer, dstOff?: number, srcOff?: number, srcLen?: number): Buffer {
        ...
        if (typeof dstLen === "number" && !dst) {
            ...
            //if (Config.GDebug) console.debug(`Oodle decompress: ${srcLen} => ${dstLen} (${seconds} seconds)`)
            return dstPointer
        }
    }
    ...
    static compress(uncompressed: Buffer, compressor: number, compressionLevel: number) {
        ...
        //const stop = Date.now()
        //const seconds = (stop - start) / 1000
        //if (Config.GDebug) console.debug(`Oodle compress: ${srcLength} => ${dstLength} (${seconds} seconds)`)
        return dst
    }
    ...
    /**
     * Downloads oodle dll if it doesn't exist
    ...
    static ensureLib() {
        try {
            if (!this.oodleLib) {
                const path = Oodle.getDLLPath()
                if (!existsSync(path))
                    throw new OodleException(`Missing oodle library '${OodleDownloader.OODLE_FILE_NAME}'!`)
